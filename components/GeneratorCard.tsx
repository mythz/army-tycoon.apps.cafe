...
import { MoneyIcon, SteelIcon, OilIcon, IncomeIcon, AmmoIcon, WheatIcon } from './icons';
      ...
      <div className="p-4 flex flex-col flex-grow">
             ...
             <div className="flex items-center gap-2 flex-wrap">
                {Object.entries(generator.production).map(([resource, value]) => (
                    <div key={resource} className="flex items-center gap-1">
                        ...
                        <span>+{formatNumber(Number(value) || 0)}/s</span>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="mt-auto">
            ...
            {purchaseOptions.map(amount => (
                ...
                className={`py-2 text-sm sm:py-1 sm:text-xs font-bold rounded-md transition-colors w-full ${purchaseAmount === amount ? 'bg-secondary text-white' : 'bg-transparent text-gray-300 hover:bg-base-100'} focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-offset-base-300 focus-visible:ring-secondary`}
                >
                x{amount}
                </button>
            ))}
            ...
            <button
                ...
                className="w-full bg-secondary/80 text-white font-bold py-2 px-4 rounded-lg transition-all duration-200 enabled:hover:bg-secondary enabled:active:scale-95 disabled:bg-gray-600 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-base-200 focus-visible:ring-secondary"
                >

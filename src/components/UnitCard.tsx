...
import { PowerIcon, MoneyIcon, SteelIcon, OilIcon, CustomizeIcon } from './icons';
  ...
  return (
    <div className="bg-base-200 border border-base-300 rounded-lg shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:border-primary/50 flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <p className="text-sm text-gray-400 mb-1">Owned: <span className="font-semibold text-secondary">{formatNumber(unit.owned)}</span></p>
        <div className="text-xs text-gray-400 mb-3 space-y-1">
            <span>WPN: {WEAPONS[unit.currentWeaponId].name} (Lvl {unit.weaponLevels[unit.currentWeaponId] || 1})</span>
        ...
        <div className="mt-auto">
            ...
            {purchaseOptions.map(amount => (
                ...
                className={`px-3 py-1 text-xs font-bold rounded-md transition-colors w-full ${purchaseAmount === amount ? 'bg-primary text-white' : 'bg-transparent text-gray-300 hover:bg-base-100'}`}
                >
                x{amount}
                </button>
            ))}
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className="flex-grow bg-accent text-white font-bold py-2 px-4 rounded-lg transition-all duration-200 enabled:hover:bg-green-500 enabled:active:scale-95 disabled:bg-gray-600 disabled:cursor-not-allowed"
                    >
                    Buy ({purchaseAmount === 'MAX' ? formatNumber(getMaxBuy()) : purchaseAmount})
                </button>
                <button onClick={() => onCustomize(unit)} className="bg-sky-600 p-2 rounded-lg hover:bg-sky-500 transition-colors">
